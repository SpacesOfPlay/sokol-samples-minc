...
// miprender-sapp.glsl - ported to minc @shader.

struct MiprenderSappVs_OffscreenOut {
    float4 pos;
    float3 nrm;
}

@shader vertex
MiprenderSappVs_OffscreenOut miprender_sapp_vs_offscreen(
    @attr(0) float4 in_pos,
    @attr(1) float3 in_nrm,
    @uniform float4x4 mvp
) {
    MiprenderSappVs_OffscreenOut o;
...
@shader fragment
float4 miprender_sapp_fs_offscreen(
MiprenderSappVs_OffscreenOut input
) {
    return float4{input.nrm * 0.5f + 0.5f, 1.0f};
}

struct MiprenderSappVs_DisplayOut {
    float4 pos;
    float2 uv;
}

@shader vertex
MiprenderSappVs_DisplayOut miprender_sapp_vs_display(
    @attr(0) float4 in_pos,
    @attr(1) float2 in_uv,
    @uniform float4x4 mvp
) {
    MiprenderSappVs_DisplayOut o;
    o.pos = mul(mvp, in_pos);
    o.uv = in_uv;
    return o;
}
...
@shader fragment
float4 miprender_sapp_fs_display(
MiprenderSappVs_DisplayOut input,
...
struct __anon_miprender_sapp_struct_0 {
    sg_view color;
    sg_view depth;
}
...
private struct state_t {
    ...
    struct {
        ...
        __anon_miprender_sapp_struct_0[9] att_views;
...
void init() {
    ...
    for i32 mip_level = 0; mip_level < 9; mip_level++ {
        noinit u8[32] color_label;
        noinit u8[32] depth_label;
        ...
        state.offscreen.att_views[mip_level].color = sg_make_view(&sg_view_desc{
            .color_attachment = sg_image_view_desc{.image = color_img, .mip_level = mip_level},
            .label = color_label,
        });
        state.offscreen.att_views[mip_level].depth = sg_make_view(&sg_view_desc{
            .depth_stencil_attachment = sg_image_view_desc{.image = depth_img, .mip_level = mip_level},
            .label = depth_label,
        });
    }
    state.offscreen.pip = sg_make_pipeline(&sg_pipeline_desc{
        ...
        .shader = sokol_make_shader(&miprender_sapp_vs_offscreen_shader, &miprender_sapp_fs_offscreen_shader),
    ...
    state.display.pip = sg_make_pipeline(&sg_pipeline_desc{
        ...
        .shader = sokol_make_shader(&miprender_sapp_vs_display_shader, &miprender_sapp_fs_display_shader),
        .index_type = SG_INDEXTYPE_UINT16,
...
void frame() {
    ...
    for i32 i = 0; i < 9; i++ {
        ...
        sshape_element_range_t shape = state.offscreen.shapes[i % 3];
...
vs_params_t compute_offscreen_vsparams() {
    ...
    mat44_t rym = mat44_rotation_z(vecmath_radians(state.ry));
...
vs_params_t compute_display_vsparams() {
    f32 w = sapp_widthf();
    f32 h = sapp_heightf();
    f32 scale = (vecmath_sin(cast(f32, state.time)) + 1.0f) * 0.5f;
    mat44_t proj = mat44_perspective_fov_rh(vecmath_radians(40.0f), w / h, 0.01f, 10.0f);
    mat44_t view = mat44_look_at_rh(vec3(0.0f, 0.0f, 2.5f), vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, 1.0f, 0.0f));
    mat44_t view_proj = mat44_mul_mat44(view, proj);
    mat44_t model = mat44_mul_mat44(mat44_rotation_x(vecmath_radians(90.0f)), mat44_scaling(scale, scale, 1.0f));
    return vs_params_t{.mvp = mat44_mul_mat44(model, view_proj)};
}
}
...
sapp_desc __sapp_sample_main() {
    return sapp_desc{
        ...
        .height = 600,
        .sample_count = 1,
        .window_title = "miprender-sapp.mc",
