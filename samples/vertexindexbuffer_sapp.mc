import dbgui;
import sokol_debugtext;
import vecmath;

import sokol_all;
import math;

// force high-dpi, sample entry point is __sapp_sample_main()
sapp_desc sokol_main() {
    sapp_desc d = __sapp_sample_main();
    d.high_dpi = true;
    return d;
}

// vertexindexbuffer-sapp.glsl - ported to minc @shader.

struct VertexindexbufferSappVsOut {
    float4 pos;
    float4 color;
}

@shader vertex
VertexindexbufferSappVsOut vertexindexbuffer_sapp_vs(
    @attr(0) float4 position,
    @attr(1) float4 color0,
    @uniform float4x4 mvp
) {
    VertexindexbufferSappVsOut o;
    o.pos = mul(mvp, position);
    o.color = color0;
    return o;
}

@shader fragment
float4 vertexindexbuffer_sapp_fs(
VertexindexbufferSappVsOut input
) {
    return input.color;
}


enum __enum_ATTR_cube_position {
    ATTR_cube_position = 0,
    ATTR_cube_color0 = 1,
    UB_vs_params = 0,
    __shim_end = 255,
}

// Replaces the sokol-shdc generated vertexindexbuffer-sapp.glsl.h.
struct vs_params_t {
    mat44_t mvp;
}

private struct state_t {
    f32 rx;
    f32 ry;
    sg_pipeline pip;
    sg_bindings bind;
    sg_pass_action pass_action;
}

private { state_t state; }
private {
f32[168] vertices = {
    -1.0f, -1.0f, -1.0f, 1.0f, 0.0f, 0.0f, 1.0f, 1.0f, -1.0f, -1.0f, 1.0f, 0.0f, 0.0f, 1.0f, 1.0f,
    1.0f, -1.0f, 1.0f, 0.0f, 0.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, 0.0f, 0.0f, 1.0f, -1.0f, -1.0f,
    1.0f, 0.0f, 1.0f, 0.0f, 1.0f, 1.0f, -1.0f, 1.0f, 0.0f, 1.0f, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, 0.0f,
    1.0f, 0.0f, 1.0f, -1.0f, 1.0f, 1.0f, 0.0f, 1.0f, 0.0f, 1.0f, -1.0f, -1.0f, -1.0f, 0.0f, 0.0f,
    1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 0.0f, 0.0f, 1.0f, 1.0f, -1.0f, 1.0f, 1.0f, 0.0f, 0.0f, 1.0f,
    1.0f, -1.0f, -1.0f, 1.0f, 0.0f, 0.0f, 1.0f, 1.0f, 1.0f, -1.0f, -1.0f, 1.0f, 0.5f, 0.0f, 1.0f,
    1.0f, 1.0f, -1.0f, 1.0f, 0.5f, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 0.5f, 0.0f, 1.0f, 1.0f,
    -1.0f, 1.0f, 1.0f, 0.5f, 0.0f, 1.0f, -1.0f, -1.0f, -1.0f, 0.0f, 0.5f, 1.0f, 1.0f, -1.0f, -1.0f,
    1.0f, 0.0f, 0.5f, 1.0f, 1.0f, 1.0f, -1.0f, 1.0f, 0.0f, 0.5f, 1.0f, 1.0f, 1.0f, -1.0f, -1.0f,
    0.0f, 0.5f, 1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, 0.0f, 0.5f, 1.0f, -1.0f, 1.0f, 1.0f, 1.0f,
    0.0f, 0.5f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 0.0f, 0.5f, 1.0f, 1.0f, 1.0f, -1.0f, 1.0f, 0.0f, 0.5f,
    1.0f,
};
u16[36] indices = {
    0, 1, 2, 0, 2, 3, 6, 5, 4, 7, 6, 4, 8, 9, 10, 8, 10, 11, 14, 13, 12, 15, 14, 12, 16, 17, 18, 16,
    18, 19, 22, 21, 20, 23, 22, 20,
};

void init() {
    sg_setup(&sg_desc{.environment = sglue_environment(), .logger = sg_logger{.func = slog_func}});
    __dbgui_setup();
    if sg_query_features().separate_buffer_types != 0 {
        sdtx_setup(&sdtx_desc_t{
            .fonts[0] = sdtx_font_cpc(),
            .logger = sdtx_logger_t{.func = slog_func},
        });
        state.pass_action = sg_pass_action{
            .colors[0] = {
                .load_action = SG_LOADACTION_CLEAR,
                .clear_value = {1.0f, 0.0f, 0.0f, 1.0f},
            },
        };
        return;
    }
    state.pass_action = sg_pass_action{
        .colors[0] = {
            .load_action = SG_LOADACTION_CLEAR,
            .clear_value = {0.375f, 0.125f, 0.25f, 0.0f},
        },
    };
    var indices_offset = cast(u64, sizeof(vertices));
    sg_buffer buf = sg_make_buffer(&sg_buffer_desc{
        .usage = sg_buffer_usage{
            .vertex_buffer = true,
            .index_buffer = true,
            .write_unsealed = true,
        },
        .size = cast(u64, sizeof(vertices) + sizeof(indices)),
        .label = "vertex-index-buffer",
    });
    sg_write_buffer_unsealed(&sg_write_buffer_desc{
        .src = sg_write_buffer_source{.data = sg_range{&vertices, sizeof(vertices)}},
        .dst = sg_buffer_location{.buffer = buf, .offset = 0},
    });
    sg_write_buffer_unsealed(&sg_write_buffer_desc{
        .src = sg_write_buffer_source{.data = sg_range{&indices, sizeof(indices)}},
        .dst = sg_buffer_location{.buffer = buf, .offset = indices_offset},
    });
    sg_seal_buffer(buf);
    state.bind = sg_bindings{
        .vertex_buffers[0] = buf,
        .index_buffer = buf,
        .index_buffer_offset = cast(i32, indices_offset),
    };
    state.pip = sg_make_pipeline(&sg_pipeline_desc{
        .shader = sokol_make_shader(&vertexindexbuffer_sapp_vs_shader, &vertexindexbuffer_sapp_fs_shader),
        .layout = sg_vertex_layout_state{
            .attrs[0] = {.format = SG_VERTEXFORMAT_FLOAT3},
            .attrs[1] = {.format = SG_VERTEXFORMAT_FLOAT4},
        },
        .index_type = SG_INDEXTYPE_UINT16,
        .cull_mode = SG_CULLMODE_BACK,
        .depth = sg_depth_state{.write_enabled = true, .compare = SG_COMPAREFUNC_LESS_EQUAL},
        .label = "render-pipeline",
    });
}

void frame() {
    if sg_query_features().separate_buffer_types != 0 {
        draw_fallback();
        return;
    }
    vs_params_t vs_params = compute_vsparams();
    sg_begin_pass(&sg_pass{.action = state.pass_action, .swapchain = sglue_swapchain()});
    sg_apply_pipeline(state.pip);
    sg_apply_bindings(&state.bind);
    sg_apply_uniforms(UB_vs_params, &sg_range{&vs_params, sizeof(vs_params)});
    sg_draw(0, 36, 1);
    __dbgui_draw();
    sg_end_pass();
    sg_commit();
}

void cleanup() {
    if sg_query_features().separate_buffer_types != 0 {
        sdtx_shutdown();
    }
    __dbgui_shutdown();
    sg_shutdown();
}

vs_params_t compute_vsparams() {
    f32 w = sapp_widthf();
    f32 h = sapp_heightf();
    var t = cast(f32, sapp_frame_duration() * 60.0);
    mat44_t proj = mat44_perspective_fov_rh(vecmath_radians(60.0f), w / h, 0.01f, 10.0f);
    mat44_t view = mat44_look_at_rh(vec3(0.0f, 1.5f, 4.0f), vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, 1.0f, 0.0f));
    mat44_t view_proj = mat44_mul_mat44(view, proj);
    state.rx += 1.0f * t;
    state.ry += 2.0f * t;
    mat44_t rxm = mat44_rotation_x(vecmath_radians(state.rx));
    mat44_t rym = mat44_rotation_y(vecmath_radians(state.ry));
    mat44_t model = mat44_mul_mat44(rym, rxm);
    return vs_params_t{.mvp = mat44_mul_mat44(model, view_proj)};
}

void draw_fallback() {
    sdtx_canvas(sapp_widthf() * 0.5f, sapp_heightf() * 0.5f);
    sdtx_pos(1.0f, 1.0f);
    sdtx_puts("WebGL2 doesn't support combined vertex/index buffers");
    sg_begin_pass(&sg_pass{.action = state.pass_action, .swapchain = sglue_swapchain()});
    sdtx_draw();
    sg_end_pass();
    sg_commit();
}
}

sapp_desc __sapp_sample_main() {
    return sapp_desc{
        .init_cb = init,
        .frame_cb = frame,
        .cleanup_cb = cleanup,
        .event_cb = __dbgui_event,
        .width = 800,
        .height = 600,
        .sample_count = 4,
        .window_title = "vertexindexbuffer-sapp.mc",
        .icon = sapp_icon_desc{.sokol_default = true},
        .logger = sapp_logger{.func = slog_func},
    };
}
