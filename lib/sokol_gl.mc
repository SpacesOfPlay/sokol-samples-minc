...
type __arr__sgl_matrix_t_64 = _sgl_matrix_t[64];
/*
    sgl_logger_t
...
struct _sgl_context_t {
    ...
    i32[3] matrix_tos;
    __arr__sgl_matrix_t_64[3] matrix_stack;
}
...
private {
u8[520] _sgl_shd_vs_source_glsl410 = {
...
u8[222] _sgl_shd_fs_source_glsl410 = {
    0x23, 0x76, 0x65, 0x72, 0x73, 0x69, 0x6f, 0x6e, 0x20, 0x34, 0x31, 0x30, 0x0a, 0x0a, 0x75, 0x6e,
...
private {
u8[481] _sgl_shd_vs_source_glsl300es = {
...
u8[253] _sgl_shd_fs_source_glsl300es = {
    0x23, 0x76, 0x65, 0x72, 0x73, 0x69, 0x6f, 0x6e, 0x20, 0x33, 0x30, 0x30, 0x20, 0x65, 0x73, 0x0a,
...
private {
u8[3395] _sgl_shd_vs_bytecode_metal_macos = {
...
u8[3047] _sgl_shd_fs_bytecode_metal_macos = {
    0x4d, 0x54, 0x4c, 0x42, 0x01, 0x80, 0x02, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
u8[3667] _sgl_shd_vs_bytecode_metal_ios = {
...
u8[3223] _sgl_shd_fs_bytecode_metal_ios = {
    0x4d, 0x54, 0x4c, 0x42, 0x01, 0x00, 0x02, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
u8[756] _sgl_shd_vs_source_metal_sim = {
...
u8[439] _sgl_shd_fs_source_metal_sim = {
    0x23, 0x69, 0x6e, 0x63, 0x6c, 0x75, 0x64, 0x65, 0x20, 0x3c, 0x6d, 0x65, 0x74, 0x61, 0x6c, 0x5f,
    0x73, 0x74, 0x64, 0x6c, 0x69, 0x62, 0x3e, 0x0a, 0x23, 0x69, 0x6e, 0x63, 0x6c, 0x75, 0x64, 0x65,
    0x20, 0x3c, 0x73, 0x69, 0x6d, 0x64, 0x2f, 0x73, 0x69, 0x6d, 0x64, 0x2e, 0x68, 0x3e, 0x0a, 0x0a,
    0x75, 0x73, 0x69, 0x6e, 0x67, 0x20, 0x6e, 0x61, 0x6d, 0x65, 0x73, 0x70, 0x61, 0x63, 0x65, 0x20,
...
private {
u8[1032] _sgl_shd_vs_bytecode_hlsl4 = {
...
u8[608] _sgl_shd_fs_bytecode_hlsl4 = {
...
private {
u8[1027] _sgl_shd_vs_source_wgsl = {
...
u8[588] _sgl_shd_fs_source_wgsl = {
    0x64, 0x69, 0x61, 0x67, 0x6e, 0x6f, 0x73, 0x74, 0x69, 0x63, 0x28, 0x6f, 0x66, 0x66, 0x2c, 0x20,
    0x64, 0x65, 0x72, 0x69, 0x76, 0x61, 0x74, 0x69, 0x76, 0x65, 0x5f, 0x75, 0x6e, 0x69, 0x66, 0x6f,
...
private {
u8[1668] _sgl_shd_vs_bytecode_spirv_vk = {
...
u8[792] _sgl_shd_fs_bytecode_spirv_vk = {
...
private {
u8* _sgl_shd_vs_source_dummy = "";
u8* _sgl_shd_fs_source_dummy = "";
}
} else {
}
private {
...
void* _sgl_malloc(u64 size) {
    ...
    if null == ptr {
        _sgl_log(SGL_LOGITEM_MALLOC_FAILED, 0, 2588);
    }
    return ptr;
}
    ...
    for i32 i = 0; i < SGL_NUM_PRIMITIVE_TYPES; i++ {
        ...
        if SGL_PRIMITIVETYPE_QUADS == i {
            ...
            if pip.pip[i].id == cast(u32, SG_INVALID_ID) {
                _sgl_log(SGL_LOGITEM_MAKE_PIPELINE_FAILED, 1, 2589);
...
sgl_pipeline _sgl_make_pipeline(sg_pipeline_desc* desc, sgl_context_desc_t* ctx_desc) {
    ...
    if pip_id.id != cast(u32, SG_INVALID_ID) {
        ...
        _sgl_log(SGL_LOGITEM_PIPELINE_POOL_EXHAUSTED, 1, 2589);
    }
    return pip_id;
}
...
void _sgl_init_context(sgl_context ctx_id, sgl_context_desc_t* in_desc) {
    ...
    if sg_add_commit_listener(_sgl_make_commit_listener(ctx)) == 0 {
        _sgl_log(SGL_LOGITEM_ADD_COMMIT_LISTENER_FAILED, 1, 2589);
    }
    sg_pop_debug_group();
    ctx.rgba = 0xFFFFFFFF;
    ctx.point_size = 1.0f;
...
sgl_context _sgl_make_context(sgl_context_desc_t* desc) {
    ...
    if ctx_id.id != cast(u32, SG_INVALID_ID) {
        ...
        _sgl_log(SGL_LOGITEM_CONTEXT_POOL_EXHAUSTED, 1, 2589);
    }
    return ctx_id;
}
...
void _sgl_setup_common() {
    ...
    _sgl_clear(&shd_desc, cast(u64, sizeof(shd_desc)));
    ...
    shd_desc.uniform_blocks[0].size = cast(u32, sizeof(_sgl_uniform_t));
    ...
    when defined(SOKOL_GLCORE) {
        shd_desc.vertex_func.source = cast(u8*, _sgl_shd_vs_source_glsl410);
        shd_desc.fragment_func.source = cast(u8*, _sgl_shd_fs_source_glsl410);
    } else when defined(SOKOL_GLES3) {
        shd_desc.vertex_func.source = cast(u8*, _sgl_shd_vs_source_glsl300es);
        shd_desc.fragment_func.source = cast(u8*, _sgl_shd_fs_source_glsl300es);
    } else when defined(SOKOL_METAL) {
        ...
        switch sg_query_backend() {
            case SG_BACKEND_METAL_MACOS: {
                shd_desc.vertex_func.bytecode = sg_range{
                    &_sgl_shd_vs_bytecode_metal_macos, sizeof(_sgl_shd_vs_bytecode_metal_macos),
                };
                shd_desc.fragment_func.bytecode = sg_range{
                    &_sgl_shd_fs_bytecode_metal_macos, sizeof(_sgl_shd_fs_bytecode_metal_macos),
                };
            }
            case SG_BACKEND_METAL_IOS: {
                shd_desc.vertex_func.bytecode = sg_range{
                    &_sgl_shd_vs_bytecode_metal_ios, sizeof(_sgl_shd_vs_bytecode_metal_ios),
                };
                shd_desc.fragment_func.bytecode = sg_range{
                    &_sgl_shd_fs_bytecode_metal_ios, sizeof(_sgl_shd_fs_bytecode_metal_ios),
                };
            }
            default: {
                shd_desc.vertex_func.source = cast(u8*, _sgl_shd_vs_source_metal_sim);
                shd_desc.fragment_func.source = cast(u8*, _sgl_shd_fs_source_metal_sim);
            }
        }
    } else when defined(SOKOL_D3D11) {
        shd_desc.vertex_func.bytecode = sg_range{
            &_sgl_shd_vs_bytecode_hlsl4, sizeof(_sgl_shd_vs_bytecode_hlsl4),
        };
        shd_desc.fragment_func.bytecode = sg_range{
            &_sgl_shd_fs_bytecode_hlsl4, sizeof(_sgl_shd_fs_bytecode_hlsl4),
        };
    } else when defined(SOKOL_WGPU) {
        shd_desc.vertex_func.source = cast(u8*, _sgl_shd_vs_source_wgsl);
        shd_desc.fragment_func.source = cast(u8*, _sgl_shd_fs_source_wgsl);
    } else when defined(SOKOL_VULKAN) {
        shd_desc.vertex_func.bytecode = sg_range{
            &_sgl_shd_vs_bytecode_spirv_vk, sizeof(_sgl_shd_vs_bytecode_spirv_vk),
        };
        shd_desc.vertex_func.entry = "main";
        shd_desc.fragment_func.bytecode = sg_range{
            &_sgl_shd_fs_bytecode_spirv_vk, sizeof(_sgl_shd_fs_bytecode_spirv_vk),
        };
...
void sgl_destroy_context(sgl_context ctx_id) {
    ...
    if _sgl_is_default_context(ctx_id) != 0 {
        _sgl_log(SGL_LOGITEM_CANNOT_DESTROY_DEFAULT_CONTEXT, 2, 2590);
        return;
    }
