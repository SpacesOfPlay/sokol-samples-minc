...
enum slbx_anchor {
    ...
    SLBX_ANCHOR_TOP = 1,
    SLBX_ANCHOR_BOTTOM = 2,
    SLBX_ANCHOR_LEFT = 3,
    SLBX_ANCHOR_RIGHT = 4,
    _SLBX_ANCHOR_FORCE_U32 = 2147483647,
}
...
*/
struct slbx_letterbox_desc {
    f32 content_aspect_ratio;
    slbx_anchor anchor;
    slbx_border border;
}
...
slbx_viewport slbx_letterbox(i32 width, i32 height, slbx_letterbox_desc* desc) {
    var b_top = cast(f32, desc.border.top);
    var b_bottom = cast(f32, desc.border.bottom);
    var b_left = cast(f32, desc.border.left);
    var b_right = cast(f32, desc.border.right);
    f32 cw = cast(f32, width) - (b_left + b_right);
    if cw < 1.0f {
        cw = 1.0f;
    }
    f32 ch = cast(f32, height) - (b_top + b_bottom);
    if ch < 1.0f {
        ch = 1.0f;
    }
    f32 content_aspect = desc.content_aspect_ratio == 0.0f ? 1.0f : desc.content_aspect_ratio;
    f32 canvas_aspect = cw / ch;
    f32 vp_x;
    f32 vp_y;
    f32 vp_w;
    f32 vp_h;
    if content_aspect < canvas_aspect {
        ...
        if desc.anchor == SLBX_ANCHOR_LEFT {
            ...
            vp_x = cast(f32, width) - (b_right + vp_w);
        } else {
        ...
        if desc.anchor == SLBX_ANCHOR_TOP {
            ...
            vp_y = cast(f32, height) - (b_bottom + vp_h);
        } else {
            vp_y = b_top + (ch - vp_h) * 0.5f;
        }
    }
    var vp = slbx_viewport{cast(i32, vp_x), cast(i32, vp_y), cast(i32, vp_w), cast(i32, vp_h)};
    return vp;
}
